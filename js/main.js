...
function calculateCaruselParameters() {
...
function createCarousel() {
    ...
    scene.style.height = sceneWidth + 'px';
    ...
    for (let i = 0; i < numberOfCarouselCells; i++) {
        const carouselCell = document.createElement('div');
        carouselCell.classList.add('beer-styles__carousel__cell');
        carouselCell.style.width = carouselCellWidth + 'px';
        carouselCell.style.height = carouselCellWidth + 'px';
...
function addCarouselMovement(field, carousel) {
    let rotation = 0;
    let rotationForce = 252 * Math.pow(radius, -1.2); //rotation force equation calculated experimentally
...
function rotateCarousel(carousel, rotation) {
